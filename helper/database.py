...
# (New, additive only — does not touch any existing function/field)

def set_intro(chat_id, file_path):
    dbcol.update_one({"_id": int(chat_id)}, {"$set": {"intro_path": file_path}}, upsert=True)

def set_outro(chat_id, file_path):
    dbcol.update_one({"_id": int(chat_id)}, {"$set": {"outro_path": file_path}}, upsert=True)

def del_intro(chat_id):
    dbcol.update_one({"_id": int(chat_id)}, {"$set": {"intro_path": None, "intro_enabled": False}})

def del_outro(chat_id):
    dbcol.update_one({"_id": int(chat_id)}, {"$set": {"outro_path": None, "outro_enabled": False}})

def toggle_intro(chat_id, enabled):
    dbcol.update_one({"_id": int(chat_id)}, {"$set": {"intro_enabled": enabled}})

def toggle_outro(chat_id, enabled):
    dbcol.update_one({"_id": int(chat_id)}, {"$set": {"outro_enabled": enabled}})

def set_watermark_text(chat_id, text):
    dbcol.update_one(
        {"_id": int(chat_id)},
        {"$set": {"watermark_type": "text", "watermark_text": text, "watermark_image": None}},
        upsert=True
    )

def set_watermark_image(chat_id, file_path):
    dbcol.update_one(
        {"_id": int(chat_id)},
        {"$set": {"watermark_type": "image", "watermark_image": file_path, "watermark_text": None}},
        upsert=True
    )
...
def del_watermark(chat_id):
    dbcol.update_one(
        {"_id": int(chat_id)},
        {"$set": {"watermark_type": None, "watermark_text": None, "watermark_image": None, "watermark_enabled": False}}
    )

def toggle_watermark(chat_id, enabled):
    dbcol.update_one({"_id": int(chat_id)}, {"$set": {"watermark_enabled": enabled}})

def get_extras(chat_id):
    data = dbcol.find_one({"_id": int(chat_id)}) or {}
    return {
        "intro_path": data.get("intro_path"),
        "outro_path": data.get("outro_path"),
        "intro_enabled": data.get("intro_enabled", False),
        "outro_enabled": data.get("outro_enabled", False),
        "watermark_enabled": data.get("watermark_enabled", False),
        "watermark_type": data.get("watermark_type"),
        "watermark_text": data.get("watermark_text"),
        "watermark_image": data.get("watermark_image"),
    }

# ============= End Intro / Outro / Watermark Settings ===========
