...
from helper.ffmpeg import take_screen_shot, fix_thumb, add_metadata, add_intro_outro_watermark
...
async def vid(bot, update):
    ...
    # ---------------- INTRO / OUTRO / WATERMARK (added) ---------------- #

    extras = get_extras(int(update.message.chat.id))
    wants_intro = bool(extras["intro_enabled"] and extras["intro_path"] and os.path.exists(extras["intro_path"]))
    wants_outro = bool(extras["outro_enabled"] and extras["outro_path"] and os.path.exists(extras["outro_path"]))
    wants_wm = bool(extras["watermark_enabled"] and extras["watermark_type"])

    if wants_intro or wants_outro or wants_wm:
        watermark_conf = None
        if wants_wm:
            if extras["watermark_type"] == "text" and extras["watermark_text"]:
                watermark_conf = {"type": "text", "value": extras["watermark_text"]}
            elif extras["watermark_type"] == "image" and extras["watermark_image"] and os.path.exists(extras["watermark_image"]):
                watermark_conf = {"type": "image", "value": extras["watermark_image"]}

        processed_path = file_path + ".processed.mp4"
        ok, err = await add_intro_outro_watermark(
            main_path=file_path,
            output_path=processed_path,
            intro_path=extras["intro_path"] if wants_intro else None,
            outro_path=extras["outro_path"] if wants_outro else None,
            watermark=watermark_conf,
            ms=ms,
            update_interval=10,
        )
        if ok:
            ...
            await ms.edit(
                "⚠️ Intro/Outro/Watermark add karte time error aaya, isliye normal renamed video bhej rahe hain.\n\n"
                f"`{(err or '')[:200]}`"
            )
