...
from helper.ffmpeg import take_screen_shot, fix_thumb, add_metadata, apply_intro_outro_watermark
...
async def vid(bot, update):
    ...
    # ---------------- INTRO / OUTRO CLIP + WATERMARK ---------------- #

    clip_settings = get_clip_settings(int(update.message.chat.id))
    if clip_settings["clips_enabled"] or clip_settings["watermark_enabled"]:
        processed_path = await apply_intro_outro_watermark(
            main_path=file_path,
            intro_path=clip_settings["intro_path"] if clip_settings["clips_enabled"] else None,
            outro_path=clip_settings["outro_path"] if clip_settings["clips_enabled"] else None,
            watermark_path=clip_settings["watermark_path"] if clip_settings["watermark_enabled"] else None,
            clips_enabled=clip_settings["clips_enabled"],
            watermark_enabled=clip_settings["watermark_enabled"],
            work_dir="downloads",
            status_msg=ms,
        )
        if processed_path:
